...
class TestConnectNativeModule: NSObject {
    ...
    @objc
    func goToSecondViewController (_ reactTag: NSNumber,data: String) {
        DispatchQueue.main.async {
            if let view = RNViewManager.sharedInstance.bridge?.uiManager.view(forReactTag: reactTag) {
             let storyBoard : UIStoryboard = UIStoryboard(name: "Main", bundle:nil)
                let secondVC = storyBoard.instantiateViewController(withIdentifier: "SecondScreen") as! SecondScreen
                secondVC.name = data
